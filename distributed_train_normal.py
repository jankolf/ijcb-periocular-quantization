...
def distributed_verification_training(config):
    ...
    trainset =  config.train_set #PeriocularTrain(config.file_content, flip_L_to_R=config.flip_images)
...
if __name__ == "__main__":
    ...
    file_content = get_file_content("/data/datasets/UFPR-Periocular/", protocol, DatasetType.TRAIN, config.fold)
    config.train_set = PeriocularTrain(file_content, flip_L_to_R=config.flip_images)
    config.test_set  = PeriocularTest(protocol, config.fold, flip_L_to_R=config.flip_images)
